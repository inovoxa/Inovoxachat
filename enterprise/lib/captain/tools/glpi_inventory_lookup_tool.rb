# Captain tool (read-only): consulta o inventário de TI (OCS Inventory) da empresa.
# Usa a conexão direta ao MySQL do OCS configurada em GlpiAccountConfig (multi-tenant).
class Captain::Tools::GlpiInventoryLookupTool < Captain::Tools::BasePublicTool
  ...
  param :os, type: 'string', desc: 'Filtrar por sistema operacional (ex.: Windows 10). Opcional.', required: false
  param :termo, type: 'string', desc: 'Texto para buscar por nome da máquina, usuário ou IP. Opcional.', required: false

  def perform(_tool_context, os: nil, termo: nil)
    log_tool_usage('searching', { os: os, termo: termo })
    cfg = glpi_config
